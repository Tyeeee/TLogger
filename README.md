...
**当前状态：早期开发中，还没发到公共仓库。** 现在能用的是"输出端"的第一片（见下方"现在能做什么"）。
...
把要用的模块源码加进来（`app/build.gradle.kts` 就是这么用的，可以直接照抄）：
...
```
// app/build.gradle.kts（按需挑，不是都得要）
implementation(project(":tlogger-core"))
implementation(project(":tlogger-android"))   // 输出到系统日志窗口
implementation(project(":tlogger-redact"))    // 打码（可选）
implementation(project(":tlogger-context"))   // 链路、会话号（可选）
implementation(project(":tlogger-ring"))      // 出事前的现场（可选）

lintChecks(project(":tlogger-lint"))          // 写代码时的隐私检查（可选）
```
...
1. **不强制全局单例**——实例归创建它的人管：`Logging(config)` 想建几个建几个，互不干扰。
   门面 `TLogger` 是**可选的**全局便利入口（让"在任意地方打一行日志"不用传引用），
   你可以完全不用它；它保存的是一个**可以被显式替换的引用**，不是一份谁都能改的全局配置。
   这一点之前写得不准确（写成"不做全局单例"，但门面确实是全局的），这里如实区分。
2. **库代码不许自己装日志系统**——只有 App（或使用它的进程入口）才调 `install`。
   库去装全局开关，就是原则 1 说的那个坑：两个库互相覆盖，谁都说不清日志去哪了。
   库代码要用日志时：要么用调用方传进来的 `Logging` 实例，要么直接 `TLogger.logger("来源")`
   （没装就是空操作，不报错也不崩）。
3. **没配置就不采集**——默认不往外写，要么是你显式装了出口。
4. **出口出错不影响业务线程**——日志写不出去是日志的事，不该拖累应用。
5. **对外接口只暴露最小面**——已开启显式接口声明（`explicitApi()`），并且不做破坏性小版本升级。
...
| 规则 | 认什么 | 准不准 |
|---|---|---|
| `TLoggerPiiInLog` | 值来自敏感 API（输入框、设备号、位置、剪贴板、账号），或消息里**直接写着**手机号/邮箱/身份证/银行卡/JWT | 确证，基本不误报 |
| `TLoggerPiiNameInLog` | 变量名本身就像隐私：`phone`、`userId`、`deviceId`、`token`…（含从它们转手的 `val id = userId`） | **启发式**，名字像不代表一定是 |

拆成两条是刻意的：真实工程里写的是 `log.d { "手机号 $phone 提交成功" }`，这个 `phone` 只是个入参、
没有任何敏感 API 可跟——只做污点分析的话一条都不会报。但按名字判断会多一点噪音，
所以单独给一条 id，嫌吵只关它、另一条不受影响：

```
// build.gradle.kts
lint {
    disable += "TLoggerPiiNameInLog"     // 只关启发式那条
}
```

两条都是**警告**级别，逐处豁免用 `@Suppress("TLoggerPiiInLog")` / `@Suppress("TLoggerPiiNameInLog")`。

### 刻意不报的（宁可漏，不制造误报）

误报多了人就把检查关掉，那等于没有。所以这些**不报**：

- `TextView.getText()` —— 那是显示用的，不是用户输入
- 内网 IP（`192.168.x.x`）—— 几乎每行日志都有，报了会淹掉真问题
- 身份证/银行卡**不过校验位**的 —— 校验位对不上说明不是真号（演示里那个 `…1234` 就是故意不报的）
- 已经打过码的（`138****5678`）
- 名字里带 `id`/`name` 但不像隐私的（`orderId`、`count`）—— 单号到处都是 id，放进来会误报一片

认不出来的情况一律放过：**它是降低风险的手段，不等于合规**。
...
./gradlew :app:lintDebug           # 跑代码检查（能看到隐私检查项报出的问题）
```
