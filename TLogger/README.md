...
> 四条设计原则：**不做全局单例**、**没配置就不采集**、**出口出错不影响业务线程**、**对外接口只暴露最小面**。
...
分支目录（`log-context`、`log-redact`、`log-lint`、`log-bridge-*`）**等第一版真正用到时再拆**，现在不预建空模块。
...
| 显式接口声明 | 已开启（`explicitApi()`） | 对外暴露的东西必须显式声明可见性 |
